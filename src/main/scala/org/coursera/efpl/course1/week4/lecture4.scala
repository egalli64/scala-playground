...
def show1(e: Expr1): String = e match
  case Expr1.Number(n)       => n.toString
  case Expr1.Sum(e1, e2)     => s"${show1(e1)} + ${show1(e2)}"
  case Expr1.Product(e1, e2) => s"${showPar1(e1)} * ${showPar1(e2)}"
  case Expr1.Var(name)       => name

def showPar1(e: Expr1): String = e match
  case e: Expr1.Sum => s"(${show1(e)})"
  case _            => show1(e)
...
def show2(e: Expr2): String = e match
  case Expr2.Number(n)       => n.toString
  case Expr2.Sum(e1, e2)     => s"${show2(e1)} + ${show2(e2)}"
  case Expr2.Product(e1, e2) => s"${showPar2(e1)} * ${showPar2(e2)}"
  case Expr2.Var(name)       => name

def showPar2(e: Expr2): String = e match
  case e: Expr2.Sum => s"(${show2(e)})"
  case _            => show2(e)
...
enum Direction(val dx: Int, val dy: Int):
  ...
  def turnLeft: Direction = Direction.values((ordinal + 1) % 4)
...
def lecture4(): Unit =
  ...
  println("Showing expression (1): " + show1(exp1))

  val exp2 = Expr2.Sum(Expr2.Product(Expr2.Number(2), Expr2.Var("x")), Expr2.Var("y"))
  println("Showing expression (2): " + show2(exp2))
